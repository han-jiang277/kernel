...
fn duration_to_ns_saturated(d: Duration) -> i64 {
...
fn thread_cpu_cycles(thread: &Thread) -> u64 {
    #[cfg(thread_stats)]
    {
        ...
        let now = time::current_clock_cycles();
        total = total.saturating_add(now.saturating_sub(thread.start_cycles()));
        total
    }
    #[cfg(not(thread_stats))]
    {
        // Fallback when per-thread cycle accounting is disabled.
        let _ = thread;
        time::current_clock_cycles()
    }
}

fn thread_cpu_time_ns(thread: &Thread) -> i64 {
    let dur = time::from_clock_cycles(thread_cpu_cycles(thread));
    duration_to_ns_saturated(dur)
}
...
pub(crate) fn monotonic_time_ns() -> i64 {
    ...
    let dur = time::from_clock_cycles(cycles);
    duration_to_ns_saturated(dur)
}
...
fn sleep_for_ticks(ticks: usize) {
    if ticks == 0 {
        ...
        scheduler::suspend_me_for::<()>(time::Tick(ticks), None);
    }
}
...
pub(crate) fn timer_create(
    ...
    // TODO: actually create a timer and return a unique ID.
...
pub(crate) fn timer_delete(timerid: timer_t) -> c_long {
    ...
    // TODO: actually delete the timer and free resources.
    0
}
...
pub(crate) fn timer_getoverrun(timerid: timer_t) -> c_long {
    ...
    // TODO: actually get the overrun count.
    0
}
...
pub(crate) fn timer_gettime(timerid: timer_t, value: *mut itimerspec) -> c_long {
    ...
    // TODO: actually get the timer's remaining time and interval.
    0
}
...
pub(crate) fn timer_settime(
    ...
    // TODO: actually set the timer and return the old value if requested.
    0
}
